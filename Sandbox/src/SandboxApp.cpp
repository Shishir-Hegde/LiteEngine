...
public:
...

  {
	  ...
	  std::shared_ptr<LiteEngine::VertexBuffer> vertexBuffer;
	  ...
	  std::shared_ptr<LiteEngine::IndexBuffer> indexBuffer;
	  ...
	  std::shared_ptr<LiteEngine::VertexBuffer> squareVB;
	  ...
	  std::shared_ptr<LiteEngine::IndexBuffer> squareIB;
	  ...
	  m_Shader.reset(new LiteEngine::Shader(vertexSrc, fragmentSrc));
	  ...
	  m_BlueShader.reset(new LiteEngine::Shader(blueShaderVertexSrc, blueShaderFragmentSrc));
  }


  void OnUpdate(LiteEngine::Timestep ts) override
  {
	  if (LiteEngine::Input::IsKeyPressed(LE_KEY_LEFT))
		  m_CameraPosition.x -= m_CameraMoveSpeed * ts;
	  else if (LiteEngine::Input::IsKeyPressed(LE_KEY_RIGHT))
		  m_CameraPosition.x += m_CameraMoveSpeed * ts;

	  if (LiteEngine::Input::IsKeyPressed(LE_KEY_UP))
		  m_CameraPosition.y += m_CameraMoveSpeed * ts;
	  else if (LiteEngine::Input::IsKeyPressed(LE_KEY_DOWN))
		  m_CameraPosition.y -= m_CameraMoveSpeed * ts;

	  if (LiteEngine::Input::IsKeyPressed(LE_KEY_A))
		  m_CameraRotation += m_CameraRotationSpeed * ts;
	  if (LiteEngine::Input::IsKeyPressed(LE_KEY_D))
		  m_CameraRotation -= m_CameraRotationSpeed * ts;
	  ...
	  LiteEngine::Renderer::BeginScene(m_Camera);
	  ...
	  for (int y = 0; y < 20; y++)
	  {
		  for (int x = 0; x < 20; x++)
		  {
			  ...
			  LiteEngine::Renderer::Submit(m_BlueShader, m_SquareVA, transform);
		  }
	  }
  ...
  private:
	  std::shared_ptr<LiteEngine::Shader> m_Shader;
	  std::shared_ptr<LiteEngine::VertexArray> m_VertexArray;

	  std::shared_ptr<LiteEngine::Shader> m_BlueShader;
	  std::shared_ptr<LiteEngine::VertexArray> m_SquareVA;
